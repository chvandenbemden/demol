...
const DEFAULT_EPISODE_COUNT = 7; // aantal afleveringen (inclusief finale)
...
// render episode navigation links
...
// --- Overview page (players + candidates with elimination episode/date) ---
function renderOverview(){
  ...
  // Candidates overview with elimination episode/date
...
function renderEpisodePage(episode){
  ...
  // allow candidates active at start of this episode
  ...
  // finalMol set (only allow candidates active in final episode)
  ...
  // show active candidates for last episode
  const activeForFinal = getActiveCandidatesForEpisode(lastEpisode);
  activeForFinal.forEach(c => { const o = document.createElement('option'); o.value=c; o.text=c; selFinal.appendChild(o); });
  ...
  btnFinal.onclick = async () => {
    ...
    setFinalMol(selFinal.value, lastEpisode);
...
function renderAdminPage(){
  ...
  card.innerHTML = `<h2>Admin</h2><div class="small">Beheer: wachtwoord instellen, locks en finale</div>`;
  // set password
  ...
  if(!final){ scoreCard.innerHTML += `<div class="small">Finale nog niet ingesteld — zet finalMol via afleveringspagina (admin)</div>`; }
  else {
...
function calculateFinalScores(finalMol){
  ...
  // compute latest prediction per episode+player
  const latestMap = {}; // key = `${episode}::${player}`
